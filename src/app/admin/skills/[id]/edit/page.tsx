...
export default async function EditSkillPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const { data, error: queryError } = await supabase.from("skills").select("*").eq("id", id).maybeSingle();
  if (queryError) {
    console.error("[admin] gagal memuat skill:", queryError);
    return <main className="min-h-screen bg-[#0a0a0f] px-6 py-10 text-slate-200"><div className="mx-auto max-w-3xl"><Link href="/admin/skills" className="text-sm text-slate-400 hover:text-cyan-400">&larr; Semua skill</Link><section role="alert" className="mt-6 rounded-xl border border-red-500/30 bg-red-500/10 p-6 text-red-200"><h1 className="font-semibold text-white">Skill tidak dapat dimuat</h1><p className="mt-2 text-sm">Periksa koneksi Supabase, lalu muat ulang halaman. Tidak ada perubahan yang dapat disimpan saat ini.</p></section></div></main>;
  }
  if (!data) notFound();
